...
def build_module(
    ...
    # import pdb; pdb.set_trace()  # Debugging breakpoint to inspect the build process
    if group in BUILD_FNS_BY_MODULE_GROUP:
        build_fn = BUILD_FNS_BY_MODULE_GROUP[group] 
        return build_fn(config, name, **kwargs)
        # slotcontrast.module.networks.build_network(config,'two_layer_mlp')
        # slotcontrast.module.encoders.build_encoder(config, 'TimmExtractor')
        ...
        # 'backbone': {
        #     'name': 'TimmExtractor',
        #     'model': '${globals.DINO_MODEL}',"vit_small_patch14_dinov2"  change DINO_MODEL to 'dino_s_vitb14'
        ...
        #     pretrained = True,
        #     frozen = True,
        #     features = ['vit_block12', 'vit_block_keys12'],(blocks.11,blocks.11.attn.qkv)
        #     checkpoint_paht = None,
        #     model_kwargs = {'dynamic_img_size': True}
        # )
        # {
...
class Resizer:
    ...
    def __call__(
        ...
        # saycam fitst time here : inputs torch.Size([32, 3, 11, 576])]
        # saycam second time here: inputs torch.Size([32, 3, 11, 257])
        if inputs.shape[-1] == 257:
            print(f'sth wrong with {inputs.shape}')
        ...
        if self.size is None:
            if size_tensor is None: #not None
                raise ValueError("If size is unspecified, need to pass a tensor to take size from")
            size = size_tensor.shape[-2:]  #[32, 3, 3, 224, 224] -> [224, 224]
        else:
            size = list(self.size)

        if self.video_inputs:  #True
            batch, n_frames = inputs.shape[:2]   # 96, 11
