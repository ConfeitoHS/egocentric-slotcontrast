...
class SlotAttention(nn.Module):
    def __init__(
        ...
        if use_mlp:
            ...
            # self.norm_mlp = nn.LayerNorm(slot_dim)   # new_implement
        else:
            self.mlp = None
    ...
    def step(
        self, slots: torch.Tensor, keys: torch.Tensor, values: torch.Tensor,i: int, N: int,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Perform one iteration of slot attention."""
        slots = self.norm_slots(slots)
        queries = self.to_q(slots)

        dots = torch.einsum("bsd, bfd -> bsf", queries, keys) * self.scale   #B,K,P, [32, 11, 256]
        pre_norm_attn = torch.softmax(dots, dim=1)   # [32, 11, 256]
    ...
    def forward(self, slots: torch.Tensor, features: torch.Tensor, n_iters: Optional[int] = None):
        ...
        for i in range(n_iters):
            slots, pre_norm_attn = self.step(slots, keys, values, i, n_iters)
