...
parser.add_argument("--log_dir", default="./logs", help="Path to log directory")
parser.add_argument(
...
def main(args, config_overrides=None):
    ...
    if not args.verbose or args.quiet:                                               # suppress warnings if not in verbose mode
        logging.getLogger("pytorch_lightning").setLevel(logging.ERROR)
        from pytorch_lightning.utilities.warnings import PossibleUserWarning

        warnings.filterwarnings("ignore", category=PossibleUserWarning)
    if args.quiet:                                                                   # suppress all warnings if in quiet mode    
        warnings.filterwarnings("ignore", category=UserWarning)

    # Setup log path
    log_path: Optional[pathlib.Path] = None
    if not args.dry:                                                                  # no log file if in dry run mode
    ...
    # [log_path] ep. logs/test/2025-08-07-17-49-11_test_dummy_image
    ...
    # [ckpt_path] ep. logs/test/2025-08-07-17-49-11_test_dummy_image/checkpoints/step=19999.ckpt

    # Setup random seed
    if "PL_GLOBAL_SEED" in os.environ:         # in DDP setring, the main process sets the seed and pass it to other processes by PL_GLOBAL_SEED
    ...
    # [config.seed] [seed] ep. 3025492063
    ...
    # Build the dataset by config.dataset:config file.dataset; 
    ...
    # import pdb; pdb.set_trace()
    # train_metrics: None
    # val_metrics: {'ari': VideoARI(), 'image_ari': ImageARI(), 'mbo': VideoIoU(), 'image_mbo': ImageIoU()}
    # config.model = {
    #     ***'initializer': {'name': 'FixedLearnedInit','n_slots': '${globals.NUM_SLOTS}','dim': '${globals.SLOT_DIM}'}, 
        # ***'encoder': {
        #     'backbone': {'name': 'TimmExtractor','model': '${globals.DINO_MODEL}','features': ['vit_block12', 'vit_block_keys12'],'frozen': True,'pretrained': True,'model_kwargs': {'dynamic_img_size': True}}, 
        #     'output_transform': {'name': 'networks.two_layer_mlp','inp_dim': '${globals.FEAT_DIM}','outp_dim': '${globals.SLOT_DIM}','hidden_dim': '${mul: ${globals.FEAT_DIM}, 2}','layer_norm': True}
        # }, 
    #     ***'grouper': {'name': 'SlotAttention','inp_dim': '${globals.SLOT_DIM}','slot_dim': '${globals.SLOT_DIM}','n_iters': 2,'use_mlp': True}, 
    #     ***'decoder': {'name': 'MLPDecoder', 'inp_dim': '${globals.SLOT_DIM}', 'outp_dim': '${globals.FEAT_DIM}', 'hidden_dims': [1024, 1024, 1024], 'n_patches': '${globals.NUM_PATCHES}'}, 
    # 
    #     'dynamics_predictor': None, 
    #     ***'predictor': {'name': 'networks.TransformerEncoder', 'dim': '${globals.SLOT_DIM}', 'n_blocks': 1, 'n_heads': 4}, 
    #     'target_encoder': None, 
    #     ***'latent_processor': {'first_step_corrector_args': {'n_iters': 3}}, 
    #     'mask_resizers': None, 
    #     'losses': {
    #         'loss_featrec': {'name': 'MSELoss', 'pred_dims': [0, '${globals.FEAT_DIM}']}, 
    #         'loss_ss': {'name': 'Slot_Slot_Contrastive_Loss', 'pred_key': 'processor.state', 'temperature': 0.1, 'batch_contrast': True, 'patch_inputs': False, 'keep_input_dim': True}
    #     }, 
    #     'loss_weights': {'loss_featrec': 1.0, 'loss_ss': 0.5}, 
    #     'input_type': 'video', 
    #     'target_type': 'features', 
    #     'target_encoder_input': None, 
    #     'visualize': False, 
    #     'eval_mode_config': None, 
    #     'visualize_every_n_steps': 10000, 
    #     'masks_to_visualize': None, 
    #    
    #     'load_weights': None, 
    ...
    # images = torch.rand(8,3,3,224,224)
    # encoder_output = model.encoder(images)
    # 'encoder':{
    #     'features': torch.Size([32, 3, 256, 64]),
    #     'backbone_features':torch.Size([32, 3, 256, 384]),
    #     'vit_block_keys12':torch.Size([32, 3, 256, 384]),
    #     'vit_block12':torch.Size([32, 3, 256, 384])
    # },


    # for name, p in list(model.named_parameters()):
    #     print(f"{name:80s} requires_grad={p.requires_grad}")
    # import pdb; pdb.set_trace()



    callbacks = _setup_callbacks(args, config, log_path, dataset)
    #{'checkpointer':....,'lr_monitor':...}
    # import pdb; pdb.set_trace()
    ...
    # {'max_steps': 100000, 'log_every_n_steps': 2500, 'val_check_interval': 2500, 'gradient_clip_val': 0.05, 'devices': 1, 'accelerator': 'auto'}
    ...
    # trainer = pl.Trainer(max_epochs=-1, check_val_every_n_epoch=None,default_root_dir=log_path,callbacks=[callback for callback in callbacks.values()],logger=[logger for logger in loggers.values()] if loggers else False,enable_progress_bar=(not args.quiet and not args.no_interactive),enable_model_summary=not args.quiet,enable_checkpointing="checkpointer" in callbacks,**trainer_config)
    ...
    # ObjectCentricModel.validation_step → aux_forward → compute_loss → validation_epoch_end
