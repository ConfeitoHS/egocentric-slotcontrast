...
def build(config, name: str):
    if name == "FrameEncoder": #True
        pos_embed = None
        if config.get("pos_embed"): #None
        ...
        return FrameEncoder(
            ...
            # now is a vit14 generated by TimmExtractor/timm.create_model,
            # change to a dino_s_vit14, generated by TimmExtractor/eminorhan.load_model
...
class FrameEncoder(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # import pdb; pdb.set_trace()  # Debugging breakpoint to inspect the backbone features
...
class TimmExtractor(nn.Module):
    ...
    def __init__(
        ...
        # print("load model first step success")
        # import pdb; pdb.set_trace()


        # timm.create_model("vit_small_patch14_dinov2", pretrained=True, checkpoint_path=None, {'dynamic_img_size': True})

        # timm base model to feature extractor
        if self.features is not None:
            if model_name in eminorhan_utils.get_available_models():
                # import pdb; pdb.set_trace()
                all_module_names = [n for n, _ in model.named_modules() if n != '']
                # ['blocks.11', 'block.11.attn.qkv']
                features = []
                for name in self.features:
                    ...
                    if name not in all_module_names:
                        raise ValueError(
                            ...
                            f"not exist in model {model_name}. Available nodes: {all_module_names}"
                        )
                    features.append(name)
                
                # import pdb; pdb.set_trace()
                from slotcontrast.modules.my_utils import HookedFeatureExtractor, VitBlock12NoCLS
                # model = HookedFeatureExtractor(model, features)
                model = VitBlock12NoCLS(model, HookedFeatureExtractor,features)
                # import pdb; pdb.set_trace()
            else:
                ...
                # a new model where output became a dict
