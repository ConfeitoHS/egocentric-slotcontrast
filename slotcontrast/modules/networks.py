...
def build(config, name: str):
    if name == "two_layer_mlp":
        ...
        if "dim" in config:
            inp_dim = config["dim"]
            outp_dim = config["dim"]
        if "inp_dim" in config: # True FEAT_DIM
            inp_dim = config["inp_dim"]
        if "outp_dim" in config: # True SLOT_DIM
        ...
        hidden_dims = [config.get("hidden_dim", 4 * inp_dim)] # 2*FEAT_DIM
        layer_norm = config.get("layer_norm") or config.get("initial_layer_norm", False) #True
        residual = config.get("residual", False)  #False
        activation = config.get("activation", "relu") #"relu"
        final_activation = config.get("final_activation", False) #False
        weight_init = config.get("weight_init", DEFAULT_WEIGHT_INIT) #"default"
        ...
        # MLP(FEAT_DIM,SLOT_DIM,2*FEAT_DIM,True,"relu",False,False,"default",False)
